...
def read_and_process_data():
    file_path = r"e:/Contest/MCM_TASK_1-main/MCM_TASK_1-main/2026_MCM_Problem_C_Data.csv"
    # 尝试多种编码读取
    try:
...
def compare_voting_methods(df, fan_estimates):
    """
    df: 包含 season, week, contestant, judge_score 的原始数据
    fan_estimates: 你的模型预测的粉丝投票数/比例
    """
    results = []

    for (season, week), group in df.groupby(['season', 'week']):
        # 1. 计算 Rank 法 (Method 1)
        group = group.sort_values('judge_score', ascending=False)
        group['judge_rank'] = range(1, len(group) + 1)
        # 假设预测的粉丝分越高排名越前
        group['fan_rank'] = group['pred_fan_score'].rank(ascending=False) 
        group['total_rank_sum'] = group['judge_rank'] + group['fan_rank']
        # 排名和越大，越容易被淘汰
        elim_rank_method = group.loc[group['total_rank_sum'].idxmax(), 'contestant']

        # 2. 计算 Percentage 法 (Method 2)
        total_judge_sum = group['judge_score'].sum()
        group['judge_pct'] = group['judge_score'] / total_judge_sum
        # 假设粉丝分已归一化为比例
        group['fan_pct'] = group['pred_fan_score_pct'] 
        group['total_pct_sum'] = group['judge_pct'] + group['fan_pct']
        # 百分比和越小，越容易被淘汰
        elim_pct_method = group.loc[group['total_pct_sum'].idxmin(), 'contestant']

        results.append({
            'season': season,
            'week': week,
            'elim_rank': elim_rank_method,
            'elim_pct': elim_pct_method,
            'is_different': elim_rank_method != elim_pct_method
        })
...
# ===================== 7. 投票规则对比与可视化 =====================
# 1. 先准备好所需列
df['contestant'] = df['player_id']  # 用 player_id 作为选手唯一标识
# 归一化粉丝分为比例
df['pred_fan_score_pct'] = df.groupby(['season', 'week'])['pred_fan_score'].transform(lambda x: (x - x.min()) / (x.max() - x.min() + 1e-8))

# 2. 计算两种规则下每周淘汰者
compare_df = compare_voting_methods(df, None)
compare_df.to_excel("Task1_VotingMethod_Compare.xlsx", index=False)
print("✅ 两种淘汰规则结果已保存: Task1_VotingMethod_Compare.xlsx")
...
plt.subplot(1, 2, 1)
bar_width = 0.4
weeks = range(len(compare_df))
plt.bar([w - bar_width/2 for w in weeks], compare_df['elim_rank'], width=bar_width, label='排名法', alpha=0.7)
plt.bar([w + bar_width/2 for w in weeks], compare_df['elim_pct'], width=bar_width, label='百分比法', alpha=0.7)
plt.xlabel('周次 (按赛季-周排序)')
plt.ylabel('淘汰者 (player_id)')
plt.title('两种规则下每周淘汰者对比')
plt.xticks(weeks, [f"S{row['season']}-W{row['week']}" for _, row in compare_df.iterrows()], rotation=90, fontsize=8)
plt.legend()

# (2) 结果不一致的周高亮热力图
plt.subplot(1, 2, 2)
compare_df['diff'] = compare_df['is_different'].astype(int)
heatmap_data = compare_df.pivot(index='season', columns='week', values='diff')
sns.heatmap(heatmap_data, annot=True, cmap='coolwarm', cbar_kws={'label': '规则不一致(1=不同)'}, linewidths=0.5)
plt.title('两种规则淘汰结果不一致热力图')
plt.xlabel('周次')
plt.ylabel('赛季')

plt.tight_layout()
plt.savefig('Task1_VotingMethod_Compare.png', dpi=300)
print("✅ 投票规则对比图已保存: Task1_VotingMethod_Compare.png")
